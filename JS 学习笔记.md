...
编译型： 通篇翻译，翻译成文件。优点：执行快。缺点：移植性不好（不夸平台）。如：C，C++，Go。

解释型：解释一行执行一行。优点：夸平台。缺点：执行慢。如：PHP, Python, JavaScript
...
* Promise.all([p1, p2]) 两个都成功才执行 then 方法
